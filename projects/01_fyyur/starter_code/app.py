...
def venues():
  ...
  test = Venue.query.with_entities(Venue, Venue.city, Venue.state).distinct(Venue.city)
  for item in test:
    venue_city = item.city
    venue_state = item.state
    entries = Venue.query.filter(Venue.city == item.city).all()
...
def create_venue_submission():
  ...
  app.logger.debug(venue_data.seeking_talent.data)
...
def edit_artist_submission(artist_id):
  ...
  app.logger.debug(form_data.city.data)

  artist = Artist.query.get(artist_id)
  app.logger.debug(artist.name)
  try:
    ...
    app.logger.info(form_data.name.data)
    app.logger.info(artist.website_link)
    db.session.commit()
...
def shows():
  ...
  for show in shows:
    app.logger.debug(show.artist.name)
    data.append({
...
def create_show_submission():
  ...
  app.logger.debug(form_data.start_time.data)
  try:
